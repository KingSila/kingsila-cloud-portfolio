...
```text
rg-kingsila-prod (Resource Group)
├── vnet-kingsila-prod (10.10.0.0/16)
│   ├── app subnet (10.10.1.0/24)
│   └── data subnet (10.10.2.0/24)
├── nsg-app-prod (Network Security Group)
│   ├── Allow SSH (port 22)
│   └── Allow RDP (port 3389)
└── prod-route-table (Route Table)
```
...
```powershell
cd C:\source\kingsila-cloud-portfolio\infra\environments\prod
terraform init
```
...
az resource list --resource-group rg-kingsila-prod --output table
...
|---------------|------|---------|
| Resource Group | `rg-kingsila-prod` | Container for all prod resources |
| Virtual Network | `vnet-kingsila-prod` | Network isolation (10.10.0.0/16) |
| Subnet (App) | `app` | Application tier (10.10.1.0/24) |
| Subnet (Data) | `data` | Data tier (10.10.2.0/24) |
| NSG | `nsg-app-prod` | Security rules for both subnets |
| Route Table | `prod-route-table` | Custom routing configuration |
