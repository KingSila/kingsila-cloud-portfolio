...
module "wi_app_prod" {
  ...
  subject = "system:serviceaccount:app-prod:api-sa"

  tags = var.tags
}

resource "azurerm_role_assignment" "wi_app_prod_kv_central_secrets_user" {
  scope                = data.azurerm_key_vault.central.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = module.wi_app_prod.principal_id
}
